// src/components/site-header.jsx
"use client";

import { usePathname } from "next/navigation";
import { Button } from "@/components/ui/button";
import { Separator } from "@/components/ui/separator";
import { SidebarTrigger } from "@/components/ui/sidebar";

export function SiteHeader() {
  const pathname = usePathname();

  // Create a mapping from URL paths to friendly titles
  const pathToTitleMap = {
    '/dashboard': 'Dashboard',
    '/products': 'Products',
    '/sales': 'Sales',
    '/analytics': 'Analytics',
    '/team': 'Team',
    '/settings': 'Settings',
  };

  // Determine the title based on the current path, with a default fallback
  const pageTitle = pathToTitleMap[pathname] || 'Documents';

  return (
    <header
      className="flex h-[var(--header-height)] shrink-0 items-center gap-2 border-b transition-[width,height] ease-linear group-has-data-[collapsible=icon]/sidebar-wrapper:h-[var(--header-height)]">
      <div className="flex w-full items-center gap-1 px-4 lg:gap-2 lg:px-6">
        <SidebarTrigger className="-ml-1" />
        <Separator orientation="vertical" className="mx-2 data-[orientation=vertical]:h-4" />
        {/* Use the dynamic pageTitle variable here */}
        <h1 className="text-base font-medium">{pageTitle}</h1>
        <div className="ml-auto flex items-center gap-2">
          <Button variant="ghost" asChild size="sm" className="hidden sm:flex">
            <a
              href=""
              rel="noopener noreferrer"
              target="_blank"
              className="dark:text-foreground">
              More Info
            </a>
          </Button>
        </div>
      </div>
    </header>
  );
}